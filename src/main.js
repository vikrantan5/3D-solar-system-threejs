...
const materials = {
  sun: new THREE.MeshStandardMaterial({ 
    ...
    color: 0xffff00,
    // emissive: 0xffaa00,
    // emissiveIntensity: 0.3 // Adjusted for texture visibility
  }),
...
const planetGroups = planets.map(planet => {
  ...
  if (planet.name === 'saturn') {
    ...
    ringMesh.scale.setScalar(planet.ringScale || 1);
    group.add(ringMesh);
  }
...
planetGroups.forEach(planet => {
  params[`${planet.mesh.name}Scale`] = planet.scale;
  if (planet.mesh.name === 'saturn') {
    params.saturnRingScale = planet.ringScale || 1;
  }
});
...
sizeFolder.add(params, 'sunScale', 0.1, 20, 0.1).name('Sun Scale').onChange(value => {
  sunMesh.scale.setScalar(value);
  params.sunScale = value;
});
planetGroups.forEach(planet => {
  sizeFolder.add(params, `${planet.mesh.name}Scale`, 0.1, 10, 0.1)
  ...
  if (planet.mesh.name === 'saturn') {
    sizeFolder.add(params, 'saturnRingScale', 0.5, 3, 0.1).name('Saturn Rings Scale').onChange(value => {
...
lightFolder.add(params, 'pointLightPosX', -50, 50, 0.1).name('Point Light X').onChange(value => {
  pointLight.position.x = value;
});
lightFolder.add(params, 'pointLightPosY', -50, 50, 0.1).name('Point Light Y').onChange(value => {
  pointLight.position.y = value;
});
lightFolder.add(params, 'pointLightPosZ', -50, 50, 0.1).name('Point Light Z').onChange(value => {
